...
const QuickViewDialog = ({ slug, onClose }: QuickViewProps) => {
  ...
  useEffect(() => {
    if (!slug) { setProduct(null); setLinks([]); return; }
    setLoading(true);
    (async () => {
      ...
      <div className="relative bg-background w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-t-3xl md:rounded-3xl shadow-soft">
        ...
        {loading || !product ? (
          ...
          <div className="grid md:grid-cols-2">
            <div className="relative bg-secondary aspect-square md:aspect-auto md:min-h-[560px]">
              {product.cover_image_url && (
                <img src={product.cover_image_url} alt={name} className="absolute inset-0 w-full h-full object-cover" />
              )}
            </div>
